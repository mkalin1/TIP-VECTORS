...
#################   #################   #################   #################   #################   #################                   
    ...
    def returnfunc(i,j,fdict,fulldict):
        for k in np.arange(-.4,1,0.1):
            if k<fdict[i][j][2]<k+0.1:
                try:
                    fulldict[str(round(k,1))]
                except:
                    fulldict[str(round(k,1))]=list()

                anglesdists=(fdict[i][j][0],fdict[i][j][1],i)
                fulldict[str(round(k,1))].append(anglesdists)
    ...
    for i in fdict.keys():
        for j in range(0,len(fdict[i])):
            if fdict[i][j][0]==0.0:
                ...
                fulldict=returnfunc(i,j,fdict,fulldict)
